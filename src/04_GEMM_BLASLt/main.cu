...
float* copy2cuda(float* h_matrix, int row, int col) {
    ...
    float* d_matrix;
    CHECK_CUDA(cudaMalloc((void**)&d_matrix, size));
    CHECK_CUDA(cudaMemcpy(d_matrix, h_matrix, size, cudaMemcpyHostToDevice));
    return d_matrix;
}
...
void test(int m, int n, int k) {
    ...
    GEMM(d_A, d_B, d_C, m, k, n, co_a, co_b, false, false);
    CHECK_CUDA(cudaMemcpy(h_O, d_C, m * n * sizeof(float), cudaMemcpyDeviceToHost));
...
//    cudaMemcpy( d_C, h_C, m * n * sizeof(float), cudaMemcpyHostToDevice);
    ...
    GEMM(d_A_T, d_B, d_C, m, k, n, co_a, co_b, true, false);
    CHECK_CUDA(cudaMemcpy(h_O, d_C, m * n * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    cp2cuda(d_C, h_C, m, n);
    GEMM(d_A, d_B_T, d_C, m, k, n, co_a, co_b, false, true);
    CHECK_CUDA(cudaMemcpy(h_O, d_C, m * n * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    cp2cuda(d_C, h_C, m, n);
    GEMM(d_A_T, d_B_T, d_C, m, k, n, co_a, co_b, true, true);
    CHECK_CUDA(cudaMemcpy(h_O, d_C, m * n * sizeof(float), cudaMemcpyDeviceToHost));
